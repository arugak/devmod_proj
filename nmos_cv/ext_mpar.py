...
for type in ('nmos', 'pmos'):
    ...
    Tox = epsOx * 120e-6 * 120e-6 / Cgg_acc
    print('paramcap_%s={\'TOX\', %e}' % (type, Tox))
    ...
    CGD0 = (Cgg_inv_L06 - (Cgg_inv_L60 - Cgg_inv_L06) / (6.0e-6 - 0.6e-6) * 0.6e-6 ) / 120e-6 / 2.0
    CGS0 = CGD0
    CF = 0 # CFとCGD0は並列関係。CFはパラメータが設定されていないと自動的に計算された値が入ってしまうのでゼロを設定
    print('paramcap_%s={\'CGD0\', %e}' % (type, CGD0))
    print('paramcap_%s={\'CGS0\', %e}' % (type, CGS0))
    print('paramcap_%s={\'CF\', %e}' % (type, CF))

    # extract CJ CJSW
    # CVカーブがあるので、フィッティングすればMJ等も抽出できる。
    # しかしとりあえずゼロバイアス単位容量CJ, CJSWのみ実装
    if type == 'nmos':
    ...
    Cj = (Cb/Pb-Cs/Ps)/(Ab/Pb - As/Ps)
    Cjsw = (Cb/Ab - Cs/As) / (Pb/Ab - Ps/As)

    print('paramcap_%s={\'CJ\', %e}' % (type, Cj))
    print('paramcap_%s={\'CJSW\', %e}' % (type, Cjsw))
